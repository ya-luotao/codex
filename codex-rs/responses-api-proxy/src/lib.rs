...
pub struct Args {
    ...
    pub startup_info: Option<PathBuf>,
...
pub fn run_main(openai_api_key: &'static str, args: Args) -> Result<()> {
    let (listener, bound_addr) = bind_listener(args.port)?;
    if let Some(path) = args.startup_info.as_ref() {
        write_startup_info(path, bound_addr.port())?;
    }
    let server = Server::from_listener(listener, None)
        .map_err(|err| anyhow!("creating HTTP server: {err}"))?;
    let client = Arc::new(
        Client::builder()
            .user_agent("codex-responses-api-proxy/0")
            .build()
    ...
    for request in server.incoming_requests() {
        ...
        std::thread::spawn(move || {
            ...
            if let Err(e) = forward_request(&client, openai_api_key, request) {
                let _ = eprintln!("forwarding error: {e}");
            }
        });
    }
...
fn write_startup_info(path: &Path, port: u16) -> Result<()> {
    if let Some(parent) = path.parent() {
        if !parent.as_os_str().is_empty() {
            let parent_display = parent.display();
            fs::create_dir_all(parent)
                .with_context(|| format!("create_dir_all {parent_display}"))?;
        }
    }
    let info = StartupInfo { port };
...
fn forward_request(client: &Client, api_key: &str, mut req: Request) -> Result<()> {
    ...
    let ct_header = upstream_resp
        .headers()
        .get(CONTENT_TYPE)
        .and_then(|value| value.to_str().ok())
        .map(ToOwned::to_owned);
    let body = upstream_resp.bytes().context("reading upstream body")?;
    let mut out = Response::from_data(body.as_ref());
    // Mirror status code
    out = out.with_status_code(status as i32);
    if let Some(ct) = ct_header {
        if let Ok(hv) = tiny_http::Header::from_bytes(&b"Content-Type"[..], ct.as_bytes()) {
            out = out.with_header(hv);
        }
    }

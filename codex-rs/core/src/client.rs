    ...
    async fn stream_responses(&self, prompt: &Prompt) -> Result<ResponseStream> {
        ...
        for attempt in 0..=max_attempts {
            match self
                ...
                .await?
            {
                StreamAttemptDecision::Completed(stream) => {
                    return Ok(stream);
                }
                StreamAttemptDecision::Retry(directive) => {
                    if attempt == max_attempts {
                        return Err(directive.into_error());
                    }

                    tokio::time::sleep(directive.delay(attempt)).await;
                }
            }
        }
    ...
    async fn attempt_stream_responses(
        ...
        match res {
            Ok(resp) if resp.status().is_success() => {
                ...
                Ok(StreamAttemptDecision::Completed(ResponseStream {
                    rx_event,
                }))
            }
            Ok(res) => {
                ...
                Ok(StreamAttemptDecision::Retry(RetryDirective::Http {
                    status,
                    retry_after,
                }))
            }
            Err(e) => Ok(StreamAttemptDecision::Retry(RetryDirective::Transport {
                error: e.into(),
            })),
        }
    }
...
enum StreamAttemptDecision {
    Completed(ResponseStream),
...
impl RetryDirective {
    ...
    fn delay(&self, attempt: u64) -> Duration {
        ...
        match self {
            Self::Http { retry_after, .. } => {
                retry_after.unwrap_or_else(|| backoff(backoff_attempt))
            }
            Self::Transport { .. } => backoff(backoff_attempt),
        }
    }

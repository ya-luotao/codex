...
impl CodexServer {
    pub async fn new_conversation(&self, config: Config) -> anyhow::Result<NewConversation> {
        ...
        let codex_conversation = Arc::new(CodexConversation {
            codex,
            cancellation_token,
        });
        self.conversations
            .write()
            .await
            .insert(conversation_id, codex_conversation);

...
def main() -> int:
    ...
    _prepare_bin_layout(bin_dir, CODEX_TARGETS)
    ...
    with tempfile.TemporaryDirectory(prefix="codex-native-artifacts-") as artifacts_dir_str:
        ...
        install_codex_binaries(artifacts_dir, bin_dir, CODEX_TARGETS)

    fetch_rg(bin_dir, DEFAULT_RG_TARGETS)

    print(f"Installed native dependencies into {bin_dir}")
    return 0


def _prepare_bin_layout(bin_dir: Path, targets: Iterable[str]) -> None:
...
def install_codex_binaries(
    artifacts_dir: Path, bin_dir: Path, targets: Iterable[str]
) -> list[Path]:
    installed: list[Path] = []
    for target in targets:

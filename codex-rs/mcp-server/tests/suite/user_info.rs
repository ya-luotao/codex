...
async fn user_info_returns_email_from_auth_json() {
    ...
    id_token.raw_jwt = encode_id_token_with_email("user@example.com");
...
fn encode_id_token_with_email(email: &str) -> String {
    use base64::Engine;
    ...
    let payload_b64 = URL_SAFE_NO_PAD
        .encode(serde_json::to_vec(&json!({ "email": email })).expect("serialize jwt payload"));

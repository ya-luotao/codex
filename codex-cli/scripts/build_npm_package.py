...
def main() -> int:
    ...
    try:
        ...
        workflow_url = args.workflow_url or resolve_latest_alpha_workflow_url()
...
def install_native_binaries(staging_dir: Path, workflow_url: str | None) -> None:
    cmd = [sys.executable, "./scripts/install_native_deps.py"]
    if workflow_url:
...
def determine_latest_alpha_version() -> str:
    ...
    for release in releases:
        ...
        if best_key is None or key > best_key:
            best_key = key
            best_version = f"{match.group(1)}.{match.group(2)}.{match.group(3)}-alpha.{match.group(4)}"
...
def list_releases() -> list[dict]:
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"Failed to list releases: {result.stderr.strip() or result.stdout.strip()}"
        )
    try:
...
def fetch_workflow_url_for_version(version: str) -> str | None:
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"Failed to list workflow runs for {ref}: "

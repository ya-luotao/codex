...
- **Windows** delegates sandboxed exec calls to your default WSL distribution when `[windows] prefer_wsl = true`, running `wsl -e codex debug landlock ...` behind the scenes so the Linux sandbox helper enforces the policy.

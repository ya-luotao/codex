...
fn truncate_middle(s: &str, max_bytes: usize) -> (String, Option<u64>) {
    ...
    if max_bytes == 0 {
        return (String::new(), Some(est_tokens));
    }

    // Split budget between head and tail. We prefer to keep whole lines to
    // avoid producing partial numeric tokens like "73".
    let left_bytes = max_bytes / 2;
    let right_bytes = max_bytes - left_bytes;

    // Find a newline boundary at or before left_bytes.
    let prefix_end = s
        .get(..left_bytes)
        .and_then(|head| head.rfind('\n').map(|i| i + 1))
        .unwrap_or(0);

    // Find a newline boundary at or after len - right_bytes.
    let start_tail = s.len().saturating_sub(right_bytes);
    let suffix_start = s
        .get(start_tail..)
        .and_then(|tail| tail.find('\n').map(|i| start_tail + i + 1))
        .unwrap_or(s.len());

    // If no newline boundaries exist, fall back to safe UTF-8 middle truncation
    // so that we always return some head+tail content.
    if prefix_end == 0 && suffix_start == s.len() {
        // Compute UTF-8 safe cut points around left/right budgets.
        let mut left = left_bytes.min(s.len());
        while left > 0 && !s.is_char_boundary(left) {
            left -= 1;
        }
        let mut right_start = s.len().saturating_sub(right_bytes);
...
abc"#;
    ...
    fn truncate_middle_no_newlines_fallback() {
        ...
        assert!(original.is_some(), "should report original token estimate");
        assert!(out.len() <= max_bytes, "output should respect cap");
        // Should preserve some prefix and some suffix from the original.
        assert!(s.starts_with(&out[..out.len().min(4)]));
        assert!(s.ends_with(&out[out.len().saturating_sub(4)..]));
